...
object SimulationInitializer {
  ...
  @tailrec
  private def initializeElevators(amount:Int, elevatorSpecifications:List[ElevatorSpecification], default: ElevatorSpecification, createdElevators:List[Elevator] = Nil):List[Elevator] = {
    if(amount > 0){
